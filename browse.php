...
if (isset($_POST["AddStockItem"])) {
    $itemArray = array(
        ...
        "BackupImagePath" => $_POST["BackupImagePath"]
    );
...
if ($CategoryID == "") {
    ...
    $Query = "
                ...
                MATCH(stockitemname, searchdetails, marketingcomments) AGAINST (? in boolean mode) as score
                FROM stockitems SI
    ...
    mysqli_stmt_bind_param($Statement, "sii",  $SearchString, $ProductsOnPage, $Offset);
...
<div id="ResultsArea" class="Browse">
    <?php if(isset($productAddedAlert)){ ?>
    <div class="alert alert-success" role="alert">
     <span>Product successvol toegevoegd aan uw winkelmand!</span>
    </div>
    <?php } ?>
                    ...
                    <div id="StockItemFrameRight">
                        <div class="CenterPriceLeftChild">
                            ...
                            <form action="" method="POST">
                                <input type="hidden" name="StockItemName" value="<?php print($row["StockItemName"]); ?>">
                                <input type="hidden" name="StockItemID" value="<?php print($row["StockItemID"]); ?>" />
                                <input type="hidden" name="ImagePath" value="<?php print($row["ImagePath"]); ?>" />
                                <input type="hidden" name="BackupImagePath" value="<?php print($row["BackupImagePath"]); ?>">
                                <input type="hidden" name="StockItemPrice" value="<?php print(sprintf(" %0.2f", berekenVerkoopPrijs($row["RecommendedRetailPrice"], $row["TaxRate"]))); ?>" />
                                <button type="submit" name="AddStockItem" <?php if ($row['QuantityOnHand'] <= 0) { print("disabled"); } ?>><i class="fas fa-cart-plus p-2"></i></button>
                            </form>
                        </div>
                    </div>
